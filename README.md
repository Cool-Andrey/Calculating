...
### Тесты неполноценные и ломанные. Если дадите время, напишу нормальные. Спасибо за понимание!
...
6. [Примеры использования (curl'ы и не только)](#примеры-использования-)
   ...
   - [/api/v1/register](#apiv1register)
     - [Общий пример запроса/ответа](#общий-пример-запросаответа-3)
     - [curl'ы](#примеры-curlов-3)
   - [/api/v1/login](#apiv1login)
       - [Общий пример запроса/ответа](#общий-пример-запросаответа-4)
       - [curl'ы](#примеры-curlов-4)
7. [Контакты](#контакты)
...
Если вернулся код ошибки `401`, значит, что аутентификация не произошла, и напишется ошибка почему конкретно. Для ещё большей конкретизации рекомендую читать логи.
...
Регистрирует в СУБД пользователя с логином login и паролем password.
...
Код ответа `200`, если записал в СУБД данные. Тело отсутствует
...
Код ответа `500` и без тела, если внутренняя ошибка.
...
Ошибку при `422` возвращает также, как [/api/v1/calculate](#apiv1calculate)

## /api/v1/login

Осуществляет аутентификацию. Возвращает JWT токен.
...
Код ответа `401` - нет пользователя в СУБД.
...
Код ответа `500` - внутренняя ошибка
...
```json
{
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3NDcwNjgwNDd9.qL_VhxKt233KOI9i5Wj0vxSP9BdXItckzf0UmXZ5zgk"
}
```

Вариант с `422` такой же, как с [/api/v1/calculate](#apiv1calculate) 
