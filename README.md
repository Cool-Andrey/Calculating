...
Обязательно прокиньте порт через -p! В команде прокидывается порт `8080` на порт сервера `8080`. Можете первое число заменить на другой порт. Например, вы хотите прокинуть порт `1234`. Тогда это будет выглядеть так `1234:8080`
...
Данные хранятся в ОЗУ(оперативке) и все данные, которые были(выражения), пропадают бесследно после перезапуска.
...
graph TD
    ...
    S -->|Есть задачка?| O
    O -->|"Да, держи задачку / Нет задачки :("| S
    A["Транспорт агентов"] -->|"GET(дай задачку)"| S
    S -->|Держи задачку/ Нет задачки| A
    A -->|"POST(держи результат)"| S
